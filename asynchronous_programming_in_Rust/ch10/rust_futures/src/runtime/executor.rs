...
impl Executor {
        ...
        F: Future<Output = ()> + 'static,
    {
        ...
        loop {
            ...
            if task_count > 0 {
                println!(
                    "{}: {} pending tasks. Sleep until notified.",
                    name, task_count
                );
...
impl Wake for MyWaker {
    // Writing the function signature this way means that wake is only callable on
    // MyWaker instances that are wrapped in Arc.
