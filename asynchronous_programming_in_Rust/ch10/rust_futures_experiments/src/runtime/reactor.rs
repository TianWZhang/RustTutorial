...
impl Reactor {
    ...
    pub fn set_waker(&self, ctx: &Context, id: usize) {
        let _ = self
            .wakers
            .lock()
            .map(|mut w| w.insert(id, ctx.waker().clone()).is_none())
            .unwrap();
    }
    ...
    loop {
        ...
        for e in events.iter() {
            ...
            if let Some(waker) = wakers.get(&id) {
                // Since calling `wake` now consumes self, we call the version that
                // takes &self instead since we want to hold on to that waker for later.
                waker.wake_by_ref();
            }
        }
    }
