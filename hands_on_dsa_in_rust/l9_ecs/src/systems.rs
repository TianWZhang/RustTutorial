use crate::{data::*, gen::GenManager};
use crate::store::EcsStore;
use termion::{cursor, color, raw::RawTerminal};
...
pub fn collision_sys<P: EcsStore<Pos>, S: EcsStore<Strength>>(pp: &P, ss: &mut S) {
    ...
    for (og, ig) in collisions {
        ...
        // bumpee is guy who gets bumped
        let h_up = if let Some(bumpee) = ss.get_mut(ig) {
            let n = bumpee.s + 1;
            bumpee.h -= dam;
            // if the bumper manages to kill the bumpee, the bumper can eat the bumpee
...
pub fn render_sys<T: std::io::Write, P: EcsStore<Pos>, S: EcsStore<Strength>>(
    ...
    pp.for_each(|g, p| {
        if let Some(st) = ss.get(g) {
            ...
            let x = (p.x % w) + 1; //termion positions start at 1
            let y = (p.y % h) + 1;
