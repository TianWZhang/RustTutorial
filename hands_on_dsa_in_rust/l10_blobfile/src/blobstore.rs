...
/// as with the hashmap wrap this in sth to make growing work
    ...
    pub fn remove<K: Serialize>(&mut self, k: &K) -> Result<(), BlobError> {
        ...
        loop {
            ...
            if b.key_match(&s_blob) {
                //item found
                let l = b.len();
                //if next block is empty, we should merge two blocks
                if pos + l < b_end {
                    if read_u64(f)? == 0 {
                        let nlen = read_u64(f)?;
                        f.seek(SeekFrom::Start(pos))?;
                        write_u64(f, 0)?;//klen = 0, means empty
...
mod test {
    ...
    pub fn test_create_file() {
        ...
        b2.insert_only("fish", "so long and thanks for all the fish").unwrap();
